...
#weighted data
dataSvy <- svydesign(ids = ~ 1, data= data1, weights = ~ data1$weight_glm)

#construct table
tabWeighted <- svyCreateTableOne(vars = var, strata = "arm", data = dataSvy,test = FALSE)

p4 <- print(tabWeighted,  smd = TRUE, printToggle = FALSE, noSpaces = FALSE)

kable(p4, booktabs = T,format = "html") %>% 
  row_spec(0,bold=TRUE) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))

logitlastipw<-glm(time_last_exp_dich~arm+age+sex+bmi+hmw.lmw+severity_ats+sinusitis+smoker+saba_sic+dysphonia_w+dyspnea_w+nsbh.res+atopy+asthma_pre,data=df,weights = ~ df$weight_glm ,family = binomial)
