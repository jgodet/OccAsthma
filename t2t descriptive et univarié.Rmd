...
### time_last_exp_m: transformation

```{r}

ggdensity(dat2t, x = "time_last_exp_m", fill = "lightgray", title = "Délai") +
  scale_x_continuous(limits = c(-50, 100)) +
  stat_overlay_normal_density(color = "red", linetype = "dashed")

skewness(dat2t$time_last_exp_m, na.rm = TRUE)
hist(dat2t$time_last_exp_m,breaks = 500)
qqnorm(dat2t$time_last_exp_m)
qqline(dat2t$time_last_exp_m)
dat2t$time_last_exp_m_norm<-log10(dat2t$time_last_exp_m)
ggdensity(dat2t, x = "time_last_exp_m_norm", fill = "lightgray", title = "Délai") +
  scale_x_continuous(limits = c(-50, 50)) +
  stat_overlay_normal_density(color = "red", linetype = "dashed")

skewness(dat2t$time_last_exp_m_norm,na.rm=TRUE)
...
### modèle complet (11 variables 213 évènements)
```{r}


dat2treg1<-dat2t%>%drop_na(t2t,bmi,wheezing_w,dysphonia_w,laba_sic,saba_sic,baseline_fev10,nsbh.res,severity_ats,time_last_exp_m_norm,ics_sic)

mod1<-glm(t2t~bmi+wheezing_w+dysphonia_w+laba_sic+saba_sic+baseline_fev10+nsbh.res+severity_ats+time_last_exp_m_norm+ics_sic,data=dat2treg1,family = binomial)
summary(mod1)
...
mod2<-glm(t2t~dysphonia_w+bmi+time_last_exp_m_norm+saba_sic+laba_sic+nsbh.res,data = dat2treg1,family=binomial)
summary(mod2)
tbl_regression(mod2,exponentiate = TRUE)
ggcoef(mod2,exponentiate = TRUE)
vif(mod2)
```
Modèle optimal avec données manquantes que pour les variables finales (passage de 570 sujets à 623)

```{r}
dat2treg2<-dat2t%>%drop_na(dysphonia_w,bmi,laba_sic,saba_sic,nsbh.res,t2t,time_last_exp_m_norm)
mod3<-glm(t2t~dysphonia_w+bmi+saba_sic+laba_sic+nsbh.res+time_last_exp_m_norm,data = dat2treg2,family=binomial)
summary(mod2)
...
```{r}
datmice<-dat2t%>%select(t2t,bmi,wheezing_w,dysphonia_w,laba_sic,saba_sic,ics_sic,baseline_fev10,nsbh.res,severity_ats,time_last_exp_m_norm)%>%
  drop_na(saba_sic,nsbh.res,dysphonia_w,laba_sic,bmi,t2t,time_last_exp_m_norm)
