...
library(readxl)
DATA<- read_excel("EAACI - Occupational Asthma - all databases 2006-2018 - 1518-Nicolas.xlsx")
...
na_strings <- c("NA", "N A", "N / A", "N/A", "N/ A", "Not Available", "NOt available","","not available",'not available')

replace_with_na_all(data=DATA,condition = ~.x %in% na_strings)

colnames(DATA)[colnames(DATA)=="CODE_CENTER_COUNTRY"] <- "COUNTRY"
colnames(DATA)[colnames(DATA)=='CODE_CENTER_CITY']<-"CITY"
colnames(DATA)[colnames(DATA)=="ESTIMATED.RATE_.ve_SIC...."]<-"RATE.SIC.POS"
