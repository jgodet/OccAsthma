...
dat$saba_w_frequency<-factor(dat$saba_w_frequency,c("?3 times a day","1 or 2 times a day","2 or more times a week","once or less per week","never"))
...
dat$only_late<-factor(dat$only_late)
summary(dat$only_late)
tabonlate<-table(dat$only_late,dat$t250)
tabonlate
cprop(tabonlate)
...
dat$early_late<-factor(dat$early_late)
summary(dat$early_late)

taboth<-table(dat$early_late,dat$t250)
taboth
cprop(taboth)
...
tabpreo<-table(dat$pre.eosino.76,dat$t250)
tabpreo
cprop(tabpreo)
...
#descriptif
tabpreneu<-table(dat$pre_neutro.76,dat$t250)
tabpreneu
cprop(tabpreneu)
by(dat$pre_neutro.76,dat$t250,summary)
...
tabpremix<-table(dat$pre_mixed.76,dat$t250)
by(dat$pre_mixed.76,dat$t250,table)
dat$pre_mixed.76<-factor(dat$pre_mixed.76)
cprop(tabpremix)
...
tabprepauci<-table(dat$pre_pauci.76,dat$t250)
by(dat$pre_pauci.76,dat$t250,summary)
cprop(tabprepauci)
...
```{r}
tabsab<-table(dat$saba_w_frequency,dat$t250)
tabsa
cprop(tabsab)
chisq.test(tabsa)
